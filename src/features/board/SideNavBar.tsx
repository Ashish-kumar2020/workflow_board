...
const SideNavBar = () => {
  ...
  const handleCreateWorkspace = () => {
    setOpenCreateWorkspace((prev) => !prev);
    console.log("Create Workspace Button CLicked", openCreateWorkspace);
  };
  ...
  return (
    <>
      <aside className="w-64 bg-[#1E293B] border border-zinc-700 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-1">
          ...
          <button
            ...
            className="w-full text-left px-3 py-2 mt-3 text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-slate-800/30 rounded-lg font-bold text-xs flex items-center gap-1.5 transition-colors cursor-pointer"
          >
      ...
      { openCreateWorkspace && <WorkSpace onClose={() => setOpenCreateWorkspace(false)}/>}
    </>
  );
};
